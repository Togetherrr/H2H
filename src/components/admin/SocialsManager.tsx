...
function SortableRow({ link, onEdit, onDelete, renderIcon }: any) {
  ...
  return (
    <TableRow ref={setNodeRef} style={style} className={`group transition-colors hover:bg-slate-50/50 ${isDragging ? "bg-white shadow-lg" : ""}`}>
      <TableCell className="w-10">
        <button {...attributes} {...listeners} className="cursor-grab active:cursor-grabbing p-1 text-slate-300 hover:text-slate-600">
      ...
      <TableCell className="text-slate-500 truncate max-w-[300px]">
        <a href={link.url} target="_blank" rel="noreferrer" className="hover:underline text-xs">
          {link.url}
        </a>
      </TableCell>
  ...
  return (
    <div className="animate-in fade-in duration-300 relative">
      <div className="mb-8 flex items-end justify-between">
        <div>
          <h2 className="text-3xl font-light tracking-tight text-slate-950 sm:text-4xl">Social Links 2.0</h2>
        ...
        <div className="flex gap-2">
          ...
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="sm:max-w-[425px] rounded-3xl">
              <DialogHeader>
                <DialogTitle>{editingId ? "Chỉnh Sửa" : "Thêm Liên Kết Mới"}</DialogTitle>
              </DialogHeader>
              <form onSubmit={handleSaveItem} className="grid gap-6 py-4">
                <div className="grid gap-2">
                  <Label htmlFor="label">Tên Kênh (Ví dụ: Official Facebook)</Label>
                  <Input
                    id="label"
                ...
                <div className="grid gap-2">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {AVAILABLE_ICONS.map((item) => (
                      <button
                        ...
                        className={`flex flex-col items-center justify-center rounded-xl border p-2 transition-all hover:bg-slate-50 ${
                          formData.note === item.id ? "border-slate-900 bg-white ring-1 ring-slate-900 shadow-sm" : "border-slate-100 bg-slate-50/50"
                        }`}
                      >
                  ...
                  <div className="mt-2 flex items-center gap-2 p-3 rounded-xl bg-slate-50 border border-slate-100">
                    <div className="flex-1">
                      <Label htmlFor="custom-icon" className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Tìm Kiếm Icon Khác</Label>
                      <Input
                        id="custom-icon"
                        placeholder="Ví dụ: Threads, Discord..."
                        value={!AVAILABLE_ICONS.find(i => i.id === formData.note) ? formData.note || "" : ""}
                        onChange={(e) => setFormData({ ...formData, note: e.target.value })}
                        className="h-8 text-xs border-none bg-transparent p-0 focus-visible:ring-0 shadow-none"
                      />
                    </div>
                    <div className="flex size-10 items-center justify-center rounded-lg border border-slate-200 bg-white shadow-sm">
                ...
                <div className="grid gap-2">
                  <Label htmlFor="url">Địa chỉ URL</Label>
                  <Input
                    id="url"
                    type="url"
                    value={formData.url}
          ...
          <Table>
            <TableHeader className="bg-slate-50/80">
              <TableRow>
                ...
                <TableHead>Địa chỉ URL</TableHead>
