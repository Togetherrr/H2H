  ...
  try {
    ...
    if (needsSpotifyFallback) {
      ...
      if (existingItemsError) {
        ...
        const itemIds = (existingItems ?? []).map((row) => row.id).filter(Boolean) as string[]
        if (itemIds.length > 0) {
    ...
    const itemsToUpsert = [
      ...
      source_updated_at: row.type === "spotify_track"
        ? (snapshot.spotify.note?.match(/(\d{4}\/\d{2}\/\d{2})/)?.[1]?.replace(/\//g, "-") ?? null)
        : null,
    }))
    ...
    const snapshotsToUpsert = snapshotRows
      .map((row) => {
        const itemId = idByKey.get(
          `${row.type}:${row.platform_id}`
        )

        if (!itemId) {
          console.log("MISSING ITEM ID:", row)
          return null
        }
        ...
        return {
          ...
          ts: bucketTs,
          total: row.total,
          daily_kworb: row.type === "spotify_track" ? (row.daily ?? null) : null,
        }
      })
      .filter(Boolean) as Array<{
        item_id: string
